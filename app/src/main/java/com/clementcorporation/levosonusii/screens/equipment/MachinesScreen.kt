...
fun MachinesScreen(navController: NavController, lifecycleOwner: LifecycleOwner) {
    ...
    val isForkliftOperator = userInfo.operatorType == context.getString(R.string.operator_type_forklift_tile_text)
    if (isForkliftOperator) equipmentScreenViewModel.retrieveForkliftsData(userInfo, isForkliftOperator)
    else equipmentScreenViewModel.retrieveElectricPalletJacksData(userInfo, isForkliftOperator)
    val currentEquipmentId = remember {
        mutableStateOf("")
    }
    equipmentScreenViewModel.fetchCurrentEquipmentId(userInfo)
    equipmentScreenViewModel.currentEquipmentIdLiveData.observe(lifecycleOwner) {
        currentEquipmentId.value = it
    }
    BackHandler {
        ...
        Scaffold(
            ...
            Column {
                ...
                Column(
                    ...
                    LazyColumn {
                        equipmentScreenViewModel.equipmentLiveData.observe(lifecycleOwner) { equipmentList ->
                            items(equipmentList) { equipment ->
                                if (equipmentList.first() is Equipment.Forklift) {
                                    SelectableTile(
                                        title = (equipment as Equipment.Forklift).serialNumber,
                                        ...
                                        equipmentScreenViewModel.setSelectedEquipmentId(equipment.serialNumber)
                                    }
                                } else {
                                    SelectableTile(
                    ...
                    Button(
                        ...
                        onClick = {
                            equipmentScreenViewModel.updateUserEquipmentData(currentEquipmentId.value, dataStore, userInfo, voiceProfile)
